...
# Edit for python 3.x Mikolaj Mikolajczak, 2021
# Redesigned
###########################################################
...
class CalendarWidget(RelativeLayout):
    ...
    def __init__(self, scr, as_popup=False, touch_switch=False, *args, **kwargs):
        super(CalendarWidget, self).__init__(*args, **kwargs)
        self.winmen=scr
        self.as_popup = as_popup
        self.touch_switch = touch_switch
        self.prepare_data()     
        self.init_ui(self.winmen)
        
    def init_ui(self, scr):
        ...
        self.create_month_scr(self.quarter[1], scr,  toogle_today=True)
    
    def create_month_scr(self, month, winmen,  toogle_today=False):
        ...
        for week in month:
            for day in week:
                ...
                if day[2] == 0:
                    tbtn.disabled = True
                self.set_tasks(winmen, tbtn, self.active_date[1] - 1, self.active_date[2])
    ...
    def set_tasks(self,winmen, inst, month, year):
        for i in winmen.get_screen('main').normaltasks:
            already_done = []
            if month<9:
                if (i.taskdate2 == str(year)+"-0"+str(month+1)+"-"+str(inst.text) or i.taskdate2 == str(year)+"-0"+str(month+1)+"-0"+str(inst.text)) and inst.text not in already_done:
                    inst.text = inst.text + "\n" + i.taskname
                    inst.halign = 'center'
                    already_done.append(inst.text)
            else:
                if (i.taskdate2 == str(year)+"-"+str(month+1)+"-"+str(inst.text) or i.taskdate2 == str(year)+"-"+str(month+1)+"-0"+str(inst.text)) and inst.text not in already_done:
                    inst.text = inst.text + "\n" + i.taskname
                    inst.halign = 'center'
                    already_done.append(inst.text)
        for i in winmen.get_screen('main').VIPtasks:
            already_done = []
            if month<9:
                if (i.taskdate2 == str(year)+"-0"+str(month+1)+"-"+str(inst.text) or i.taskdate2 == str(year)+"-0"+str(month+1)+"-0"+str(inst.text)) and inst.text not in already_done:
                    inst.text = inst.text + "\n VIP " + i.taskname
    ...
    def go_prev(self, inst):
        ...
        if not self.sm.has_screen(prev_scr_name):
            self.create_month_scr(self.quarter[0], self.winmen)
    ...
    def go_next(self, inst):
        ...
        if not self.sm.has_screen(next_scr_name):
            self.create_month_scr(self.quarter[2], self.winmen)
